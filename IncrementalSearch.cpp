...
KeyCord::command_return_t IncrementalSearch::my_incremental_search(std::shared_ptr<Model> model, std::shared_ptr<View> view, Control& control, std::shared_ptr<Control> incremental_search_control, std::shared_ptr<Model> incremental_search_model)
{
	...
	while (m_run_loop)
	{
		...
		if (!m_quitting)
		{
			...
			if (m_start_row <= row && row <= m_start_row + height)
				_rows = control.rows(Control::change_t::VISUAL, m_start_view_row, m_start_view_row + height);
			else
			{
				//control.change_view(row - height / 2, view_col, height);
				_rows = control.rows(Control::change_t::VISUAL, row, row + height);
			}
			view->update(_rows, control.get_modes()[0], view_col);
			IncrementalSearchMode& ism = dynamic_cast<IncrementalSearchMode&>(*(control.get_modes()[1]));
			ism.set_search(search_string);
			view->update(_rows, control.get_modes()[1], view_col);
		}
...
KeyCord::command_return_t IncrementalSearch::my_incremental_search_insert(std::shared_ptr<Model> model, std::shared_ptr<View> view, Control& control, wchar_t wc)
{
...
KeyCord::command_return_t IncrementalSearch::my_incremental_search_enter(std::shared_ptr<Model> model, std::shared_ptr<View> view, Control& control)
{
	control.exit();
	m_run_loop = false;
	return make_tuple(&my_empty_undo, false);
...
KeyCord::command_return_t IncrementalSearch::my_incremental_search_again(std::shared_ptr<Model> model, std::shared_ptr<View> view, Control& control)
{
	intptr_t row, col;
	m_delta_col = 1;
	control.exit();
...
KeyCord::command_return_t IncrementalSearch::my_incremental_search_quit(std::shared_ptr<Model> model, std::shared_ptr<View> view, Control& control)
{
...
KeyCord::command_return_t IncrementalSearch::my_incremental_search_backspace(std::shared_ptr<Model> model, std::shared_ptr<View> view, Control& control)
{
	return make_tuple(&my_empty_undo, false);
}